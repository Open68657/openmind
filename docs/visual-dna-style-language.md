...
1. **האדם נטול-הפנים כמגיש — במנעד קומפוזיציות רחב.** הרפרטואר כולל: טורסו שחובק
   אשכול-שפע (סנטר-עד-מותן) · דמות מלאה בהליכה/עמידה עם סל (חתוכה מתחת לסנטר) · יד
   אחת שמחזיקה מיכל · סטילס של מיכל/מגירה בלי אדם כלל · וגם ידיים מקצוות הפריים —
   אבל זו **רק אופציה אחת מתוך המנעד, לא ברירת המחדל**. תמיד: בלי פנים, או דדפאן בלי
...
4. **אור יום קשה וישיר.** תאורה מסחרית בהירה עם אופי כמעט-פלאשי: צללים חדים
   ומוגדרים על הרקע, היילייטים מבריקים על פלסטיק וזכוכית. אופטימי, אף פעם לא
   דרמטי/אפל — אבל גם לא רך ושטוף.
5. **פוטוגרפיה אמיתית ומוחשית**, מראה מדיום-פורמט, מרקמים קריאים (קליפה, אדים, אריג,
   פלסטיק) — **לא איור, לא תלת-ממד.**
6. **פלטה רוויה ואופטימית — צבעי סוכריה עזים, לא פסטל שטוף.** רקע בגוון בולט;
   האובייקטים בצבעי candy רוויים ומלאים (מנטה, לבנדר, ורוד, תכלת, שרטרז) + פופ חם
   (חרדל/ochre, קורל). לבוש, כשמופיע: נורמקור עמום (זית, קמל, אפור, לבן) — הניגוד
   בין הלבוש העמום לאובייקטים העזים הוא חלק מהשפה.
...
### מלתחת הקו (Wardrobe DNA)
- **ז'אנר:** נורמקור-וורקוויר יומיומי, מגוהץ ומסודר — צ'ור-ג'קט כותנה עבה, טי חלק,
  חולצת כפתורים ג'ינגהם חיוורת (אפרסק/שמנת), סריג דק, צ'ינו, ג'ינס בשטיפה בינונית.
  שום פריט הצהרתי, שום ספורטוור, שום מותג.
- **תפקיד:** הלבוש הוא הפויל העמום שמבליט את האובייקטים הצבעוניים — לעולם לא מתחרה בהם.
- **שכבות עם פרט אגבי:** שרוול משובץ מציץ מתחת לשרוול הג'קט, מכפלת מקופלת בגוון אחר.
- **ידיים: תמיד חשופות.** ציפורניים טבעיות מטופחות, מקסימום טבעת נישואין דקה.
  לעולם לא כפפות, שעונים או צמידים — ובנושאי ספורט/מקצוע יש לכתוב זאת במפורש
  בפרומפט ("bare hand, no gloves"), אחרת המודל משלים ציוד מקצועי.
- **נעליים:** סניקרס עור לבנות פשוטות + גרביים לבנים גבוהים.
- **אקססוריז מותרים:** כובע מצחייה בז', משקפי מתכת עדינים. זהו.
...
- `[PRESENTATION]` — **גוון בין הקומפוזיציות, אל תחזור על אותה תבנית:** טורסו חובק
  אשכול-שפע / דמות מלאה בהליכה עם סל (חתוכה מתחת לסנטר) / יד אחת מחזיקה מיכל שקוף /
  מגירה-מיכל בלי אדם / ידיים מקצוות הפריים (במשורה) / חפץ מרחף לבדו.
...
>
> **PALETTE:** saturated and optimistic. Objects in vivid, fully-saturated candy colors
> (mint, lavender, blush pink, sky blue, chartreuse, peach) — punchy, not washed-out —
> plus one warm accent pop (mustard/ochre, coral, orange). Muted normcore wardrobe
> against vivid objects is part of the language.
>
> **LIGHT:** hard direct daylight-commercial lighting, near-flash character; strong crisp
> defined cast shadows on the backdrop, glossy specular highlights on plastic and glass.
> Punchy, never moody — and never soft/washed.
>
...
## 4.5 העיקרון החשוב ביותר: לביים מחדש, לא לשחזר

**אסור להעתיק קומפוזיציה מהרפרנסים ולהחליף בה אובייקטים.** לכל נושא מביימים
מיזנסצנה חדשה שנובעת מהנושא עצמו, ורק העקרונות (אור קשה, צבעוניות עזה, בלי פנים,
נגטיב-ספייס, סוריאליזם עדין, מגע אנושי) נשמרים. דוגמאות לבימוי נושא-ספציפי:
- **יוגורט:** 2–3 גביעים במרכז, יד נכנסת מלמעלה עם כפית, ומתוך היוגורט מתפרצים
  באוויר קוקוס, שיבולת שועל ואוכמניות — קפואים באמצע תנועה.
- **קוסמטיקה:** צנצנת קרם פתוחה במרכז עם ספירלת קרם שעולה ממנה; שתי אצבעות
  מחזיקות את המכסה מרחף מעליה; סביבה טבעת אלמנטים תלויים (טיפת סרום, פרוסת אלוורה).
- **תיק/לימודים:** תיק עומד פתוח במרכז, וקשת מסודרת של חפצים מרחפת החוצה ממנו
  כאילו "מעלה קבצים"; יד נכנסת מפינת הפריים לתפוס את העליון.

הכלל: קודם שואלים "מה ההתרחשות הקטנה והמפתיעה שנכונה לנושא הזה?" — ורק אז
מלבישים עליה את כללי הארט-דיירקשן.

**וגם גיאומטריית הריחוף מתגוונת.** ניסוחי פרומפט כמו "neat ascending arc" /
"tidy queue" / "line" מתורגמים אצל המודל כמעט תמיד לשורה אלכסונית עולה — תבנית
חוזרת בדלת האחורית. לכל בימוי בוחרים גיאומטריה אחרת וכותבים אותה במפורש:
פיזור-קונסטלציה בגבהים ועומקים שונים / טבעת-מסלול סביב הגיבור / עמודה אנכית /
גריד שטוח ומסודר (דדפאן מאוד) / התפרצות רדיאלית / נפילה קפואה / או בלי ריחוף.
